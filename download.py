...
class Download:
    ...
    def __init__(self,directoryEntry,lang):
    ...
    def start(self,mainframe,queue_frame,window):
        ...
        self.lbl_Download_Song = ttk.Label(frm_Download_Song)
        self.lbl_Download_Playlist = ttk.Label(frm_Download_Playlist)
        self.lbl_Download_Song.grid(row=0,column=0)
        self.lbl_Download_Playlist.grid(row=0,column=0)
        ...
        self.btn_Download_Song.configure(command=lambda:gfunc.createSong(queue_frame,self.entry.get(),self.txt_Download_Song,self.song_list,self.lang))
        self.btn_Download_Playlist.configure(command=lambda:gfunc.createPlaylist(queue_frame,self.entry.get(),self.txt_Download_Playlist,self.song_list,self.lang))

        #Start command on enter key
        self.txt_Download_Song.bind('<Return>',lambda e:gfunc.createSong(queue_frame,self.entry.get(),self.txt_Download_Song,self.song_list,self.lang))
        self.txt_Download_Playlist.bind('<Return>',lambda e:gfunc.createPlaylist(queue_frame,self.entry.get(),self.txt_Download_Playlist,self.song_list,self.lang))
    ...
    def changeTheme(self,theme):
        if theme == "dark":
            ...
            self.btn_Download_Song.configure(background="#C8C6C6",activebackground="#C8C6C6",foreground="black",activeforeground="black")
            self.btn_Download_Playlist.configure(background="#C8C6C6",activebackground="#C8C6C6",foreground="black",activeforeground="black")
        elif theme == "light":
    ...
    def changeLang(self,lang):
        if lang == "it":
            self.lbl_Download_Song.configure(text = getLabelIt("song"))
            self.lbl_Download_Playlist.configure(text = getLabelIt("playlist"))
        elif lang == "en":
            self.lbl_Download_Song.configure(text = getLabelEn("song"))
            self.lbl_Download_Playlist.configure(text = getLabelEn("playlist"))
        for elem in self.song_list:
            elem.changeLang(lang)
        self.lang = lang
